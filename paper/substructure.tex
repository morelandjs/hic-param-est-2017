...
\usepackage[labelfont={color=theblue}, justification=raggedright, labelsep=quad]{caption}
...
\begin{figure}
  ...
  \caption{
    ...
    Schematic of plausible proton shapes.
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
    \label{fig:thickness} Effect of nucleon substructure on the nuclear thickness function $T(x, y) \equiv \int dz\, \rho(x, y, z)$ of a $^{208}\mathrm{Pb}$ nucleus.
...
\fig[t]{viscosity_dof}{
Degrees of freedom in the temperature dependent shear and bulk viscosity parametrizations.
...
\fig*[t]{observables_design}{
Hybrid model calculations compared to experimental data for $p$-Pb collisions (left column) and Pb-Pb collisions (right column) at $\sqrts=5.02$~TeV.
Colored lines are model calculations (no emulator) for each of the 500 design points.
Points with error bars are experimental data from ALICE and CMS \note, with statistical and systematic errors added in quadrature.
}

\fig*[t]{observables_posterior}{
Calibrated emulator predictions compared to experimental data for $p$-Pb collisions (left column) and Pb-Pb collisions (right column) at $\sqrts=5.02$~TeV.
Colored lines are emulator predictions for $n=100$ different parameter sets randomly sampled from the Bayesian posterior.
Points with error bars are experimental data from ALICE and CMS \note, with statistical and systematic errors added in quadrature.
}
...
Points with error bars are the experimental data with statistical uncertainties, and gray bands their corresponding systematic uncertainties, from ALICE and CMS \note.
The ratio axes show the ratio of model over data where where available with gray bands indicating $\pm 10\%$.
}
...
\fig{posterior_proton_shape}{
  Posterior distribution for the constituent sampling radius $\R$ and constituent width $\w$.
  The white trapezoidal region (lower-right) covers the initial range of allowed values for $\R$ and $\w$, while the gray triangular region (upper-left) indicates values which are excluded by the prior $\w > \R$.
  The posterior distribution, shown in blue, indicates the preferred values for $\R$ and $\w$ determined by the analysis.
}
