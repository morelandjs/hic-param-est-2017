...
\newcommand{\dTb}{\Delta T_\text{bulk}}
\newcommand{\Tb}{T_\text{bulk}}
...
To this point, we note that the arguments for the generalized mean ansatz are quite general \cite{Moreland:2015oya}, and that the energy density $dE/d\eta_s$ and entropy density $dS/d\eta_s$ share many similar qualities.
...
\begin{equation}
  ...
  (\zeta/s)(T) = \frac{(\zeta/s)_\text{max}}{1 + \left(\dfrac{T - T_\text{bulk}}{\Delta T_\text{bulk}}\right)^2},
\end{equation}
described by a tunable maximum $(\zeta/s)_\text{max}$, width $\dTb$, and location parameter $\Tb$.
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    $\dTb$            & Width of $\zeta/s$ peak              & 0--0.1 GeV      \\
    $\Tb$             & Temperature of $\zeta/s$ maximum     & 150--200 MeV    \\
...
Consider, for example, a single multiplicity bin $[\nch^\text{low}, \nch^\text{high}]$ which selects events from a minimum bias event sample with $p_T^\text{min} < p_T < p_T^\text{max}$ and $|\eta| < \Delta \eta$.
...
\end{equation}
where $E_T \propto (dE_T/d\eta_s) \vert_{\eta_s=0}$ is the midrapidity transverse energy of a single event, and $\langle E_T \rangle$ is the corresponding average over the minimum bias event sample.
...
\begin{table}[t]
  ...
  \begin{ruledtabular}
    ...
    \begin{tabular}{llll}
      ...
      $n_c$      & \est{6.3}{3.1}{2.7}           & $\dTb$           & \est{0.033}{0.033}{0.048} GeV     \\[1.1ex]
      $v$       & \est{0.51}{0.18}{0.26} fm      & $\Tb$            & \est{0.176}{0.020}{0.024} GeV     \\[1.1ex]
...
The \trento\ normalization factor ${\text{Norm} = 20.2^{+2.2}_{-2.2}}$ and energy deposition parameter $p=0.013^{+0.144}_{-0.155}$ are well constrained by the present analysis.
...
As the energy deposition parameter $p$ trends toward positive (negative) infinity, particle production scales like the maximum (minimum) of local nuclear overlap density.
...
Continuing down the diagonal in Fig.~\ref{fig:posterior}, we see that the constituent sampling radius $r=0.88_{-0.23}^{+0.25}$~fm, and the constituent width $v=0.51_{-0.18}^{+0.26}$~fm.
Their joint posterior distribution is shown in Fig.~\ref{fig:posterior_proton_shape}, illustrating the constraining power of the analysis.
Recall that the parameter $r$ controls the dispersion of the constituent positions, while $v$ varies each constituent's Gaussian width.
We emphasize that $r$ is model parameter, not a physical one.
It describes the radius of the constituent position sampling distribution, \emph{not} the average proton radius, a quantity that's measured in the proton's center-of-mass frame.
For example, when $n_c=1$, our model describes each nucleon as a single Gaussian of width $v$, yet the parameter $r$ could nevertheless assume any value within its prior.
Despite this idiosyncrasy, one can easily calculate a physical nucleon width \emph{ex post facto}, given specific values for $r$, $v$, and $n_c$.
...
For example, using the MAP estimate $r=0.88$~fm, $n_c=6$, and $v=0.51$~fm, we obtain a Gaussian nucleon width $w = 0.88$~fm, corresponding to a three-dimensional root-mean-square radius $R_p = \sqrt{3} w = 1.52$~fm.
This estimate is consistent with the nucleon width extracted by a similar Bayesian analysis lacking nucleon substructure \note.
...
The posterior free-streaming time $\taufs=0.37_{-0.27}^{+0.33}\ \fmc$ obtained in this work is significantly smaller than the previous estimate $\taufs=1.16_{-0.25}^{+0.29}\ \fmc$ quoted in Ref.~\cite{Bernhard:2018hnz}.
...
\begin{table}
  ...
  \begin{ruledtabular}
    \begin{tabular}{ll@{\hspace{2em}}ll}
      ...
      $r$      & 0.81 fm        & $\dTb$            & 0.024 GeV      \\
      $v$      & 0.43 fm        & $\Tb$             & 175 MeV        \\
