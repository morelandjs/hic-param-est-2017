...
\newcommand{\n}{n_\mathrm{c}}
\newcommand{\R}{R_\mathrm{c}}
\newcommand{\w}{w_\mathrm{c}}
\newcommand{\dmin}{d_\mathrm{min}}
\newcommand{\T}{\tilde{T}}
\newcommand{\x}{\mathbf{x}}
\newcommand{\y}{\mathbf{y}}
\newcommand{\yexp}{\mathbf{y_\mathrm{exp}}}
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{Effect of nucleon substructure on the nuclear thickness function $T(x, y) \equiv \int dz\, \rho(x, y, z)$ of a $^{208}\mathrm{Pb}$ nucleus. The nucleus on the left has Gaussian nucleons of width $0.88$~fm, while the nucleus on the right has composite nucleons, each containing six Gaussian constituents of width $0.41$~fm.}
  \label{fig:thickness}
\end{figure}
...
Given values for the parameters $\x$, the model may be used to predict a vector of experimental observables $\y$.
For example, $\y$ might be a vector of fixed length which includes charged particle yields in different centrality bins.
The physics model thus represents a vector-valued function $f(\x) = \y$
which maps the parameter values $\x$ to the observable state $\y$.

The goal of this work is to estimate the true model parameters $\x_\star$, given some evidence that our model ${f: X \mapsto Y}$ describes the experimental data $\yexp$ for the simulated observables $\y$.
According to Bayes' theorem, the probability for the true model parameters $\x_\star$ is
\begin{equation}
  \label{eq:bayes}
  P(\x_\star|f, \yexp) \propto P(f, \yexp|\x_\star) P(\x_\star).
\end{equation}
The left-side of this expression is the \emph{posterior}: the probability that $\x_\star$ is the correct value given evidence from the model $f: X \mapsto Y$ and the experimental data $\yexp$.

On the right-side, the \emph{likelihood} function $P(f, \yexp | \x_\star)$ is the compatibility of the experimental evidence $\yexp$ with the hypothetical model parameters $\x_\star$, and the \emph{prior} probability $P(\x_\star)$ is the estimate of the probability of $\x_\star$ in the absence of evidence $\yexp$.

\begin{equation}
  P(H1 | E, H2) \propto P(E | H1, H2)\, P(H1 | H2)
\end{equation}
There are three types of events,
\begin{itemize}
  \item $H1$: hypothesis that the model parameters $\x_\star$ are the true model parameters.
  \item $H2$: hypothesis that nuclear collisions are describable using the hybrid transport model formulated in this work.
  \item $E$: experimental evidence provided by data $\yexp$.
\end{itemize}

\subsection{Parameter design}

We create a scaffolding of the parameter space and distribute $n=500$ parameter points in our 15-dimensional parameter space using a Latin hypercube design \note.
The algorithm maximizes the

We assume a flat prior distribution for each model parameter $x \in \x$ described by the range of values shown in Tbl.~\ref{tab:design}.
The minimum and maximum values of the parameters are conservative to avoid clipping the calibrated posterior; for example, previous analyses of \trento\ model found $p \sim 0$, but we use the prior $-1 < p < 1$ to account for physics differences in the present model which might modify its posterior.

\subsection{Gaussian process emulator}

\begin{table}
  \caption{Input parameter ranges for the initial condition and hydrodynamic models.}
  \begin{ruledtabular}
  \begin{tabular}{lll}
