...
We then train a set of independent GPE's,
\begin{align}
  \label{eq:gpe}
  z_1 &= \text{gp}_1(\x), \nonumber \\
  z_2 &= \text{gp}_2(\x), \nonumber \\
  ...
  z_k &= \text{gp}_k(\x),
\end{align}
to predict the principal components (coefficients) $(z_1, \dots, z_k)$ of the first $k$ principal component vectors as a function of the model parameters $\x$ which vary across the design $X$.
We use $k=7$ principal components when emulating the $p$-Pb system and $k=8$ principal components when emulating Pb-Pb, chosen to describe 99.5\% of the total observed variance of each system.
...
Once the GPE's \eqref{eq:gpe} are trained, we can predict the model observables $\y_m$ at parameter point $\x$ by transforming the vector of principal components
\begin{equation}
...
\begin{equation}
\text{Cov}(z_i, z_j) = \text{diag}[(\delta z_{1})^2, (\delta z_2)^2, \dots, (\delta z_k)^2 ],
\end{equation}

\begin{fullpage}
{\centering\includegraphics{observables_pbpb}\par}
\captionof{figure}{Simulated observables compared to experimental data for Pb-Pb collisions at $\sqrts=5.02$~TeV. Top row: explicit model calculations (no emulator) for each of the 500 design points; bottom row: emulator predictions for $n=100$ random samples drawn from the posterior. Points with error bars are experimental data from ALICE with statistical and systematic errors added in quadrature \note.}
\vspace{.5cm}
{\centering\includegraphics{observables_ppb}\par}
\captionof{figure}{Same as Fig.~\ref{fig:obs_pbpb} but for $p$-Pb collisions at $\sqrts=5.02$~TeV. Note that multiplicity bins are used for mean $p_T$ and flow cumulant observables to match the bins used by experiment. Experimental data are from ALICE and CMS \note.}
\end{fullpage}
...
We now direct our attention to Fig.~\ref{fig:posterior} which shows the posterior distribution of the model input parameters.
...
The present distribution has 15 dimensions, one for each parameter listed in Tbl.~\ref{tab:design}, and so we sample the distribution and histogram the samples to project the distribution onto a lower number of dimensions for visualization purposes.
Each diagonal panel in the figure is the distribution of a \emph{single} model parameter (marginalized over all other parameters), and each lower-diagonal panel is the joint distribution of a \emph{pair} of model parameters (marginalized over all other parameters).
The full posterior distribution includes a wealth of information.
Here we focus on the most important features...
...
The simulation time of a single \texttt{VISH2+1} event scales like ${\sim}n_x^3$ since $dx \propto d\tau$, and thus our ``pre-run'' event requires only ${\sim}1/27$th the time of a production event.
...
It is not immediately clear why this would be the case, but the MAP observables in Fig.~\note\ are in good agreement with their emulator predictions which suggests it should not be a grave concern.
