...
The effect of this addition nucleon substructure on the nuclear thickness functions is visible in Fig.~\ref{fig:thickness}
...
\begin{figure}
  ...
  Posterior distribution for the nucleon width $w$ and constituent width $v$. The enclosed trapezoidal region covers the prior range of allowed values for $w$ and $v$. The posterior distribution, shown in blue, indicates the preferred values for $w$ and $v$ determined by the analysis.}
\end{figure}
...
Lastly, we compute the symmetric cumulants $\mathrm{SC}(m,n)$ for the Pb-Pb collision system at $\sqrts=5.02$~TeV which quantify correlations between event-by-event fluctuations of the flow harmonics of different order \cite{Bilandzic:2013kga, ALICE:2016kpq},
\begin{align}
...
We observe that the gap between $\mathrm{SC}(3,2)$ and $\mathrm{SC}(4,2)$ is generally wider in the present analysis than in Ref.~\cite{Bernhard:2018hnz}, as is the gap between the normalized symmetric cumulants $\mathrm{NSC}(3,2)$ and $\mathrm{NSC}(4,2)$.
...
\begin{figure}
  ...
  \caption{Model calculations of the symmetric cumulants (top) and normalized symmetric cumulants (bottom) for Pb-Pb collisions at $\sqrts=5.02$~TeV using the \emph{maximum a posteriori} (MAP) parameters. The solid lines are the MAP estimate of the present analysis (with nucleon substructure), and the dashed lines are the MAP estimate of Ref.~\cite{Bernhard:2018hnz} (without nucleon substructure) which was calibrated on Pb-Pb observables at $\sqrts=2.76$ and 5.02~TeV. In general, most model parameters are somewhat different between the two studies.}
  \label{fig:flow_corr}
\end{figure}
...
These expectations were upended, however, when long-range multiparticle correlations were detected in high-multiplicity $p$-Pb collisions and found to be similar in magnitude to those observed in Pb-Pb collisions \note.
...
For the QGP initial conditions, we employed a modified version of the \trento\ model which adds new parameters to vary the fluctuating size and shape of each nucleon.
...
With the full evolution model in hand, we applied Bayesian methods which were developed to estimate the parameters of computationally intensive models \note.
...
The excellent quantitative agreement of the model calculation with the experimental data is strong evidence for a unified hydrodynamic description of $p$-A and A-A collisions.
...
Second, we obtained a posterior distribution for the model input parameters in Fig.~\ref{fig:posterior} and reported quantitative estimates for the model input parameters in Tbl.~\ref{tab:post_param}.
Here we summarize our key findings about the model:
\begin{enumerate}[itemsep=0pt, leftmargin=2\parindent]
  \item
    Using the \trento\ initial condition model, we find that initial entropy deposition (or particle production) scales like the geometric mean of local participant nuclear densities, in agreement with a number of previous estimates \note.
  \item
  ...
  \item
    ...
    This agrees with a similar Bayesian analysis which was calibrated to Pb-Pb bulk observables at $\sqrts=2.76$ and 5.02~TeV using a hybrid model without nucleon substructure \note.
    We note that our result is in tension an estimate for the effective nucleon width based on the gluon distribution extracted from HERA data \note.
  \item
    We obtain an estimate $v=0.46_{-0.15}^{+0.21}$~fm for the Gaussian width of individual constituent hot spots inside the nucleon is which is significantly larger than the length scales typically associated with nucleon substructure.
    This result may help explain the seemingly unreasonable success of hydrodynamics in small collision systems.
  \item
  ...
  \item
    The posterior estimates for the temperature dependence of the QGP specific shear and bulk viscosities
\end{enumerate}

The present study would benefit from a number of improvements.
Perhaps the most obvious improvement would be to add missing experimental data to the calibration once it becomes available.
For example, the present calibration is missing the Pb-Pb mean $p_T$, as well as the transverse energy, identified particle yields, and mean $p_T$ fluctuations of both collision systems.
These should certainly influence the quality of the combined fit and the posterior estimates for the model parameters.

Similarly, the analysis would greatly benefit from additional beam energies and collision systems.
Notably missing are measurements of $p$-Au, $d$-Au and $^3$He-Au collisions at RHIC \note, which would provide an additional constraint on the parameters and more stringent tests of the calibrated model predictions.
...
\begin{acknowledgments}
  ...
  JSM, JEB and SAB are supported by the U.S.\ Department of Energy Grant No.\ DE-FG02-05ER41367 and National Science Foundation Grant No.\ NSF-ACI-1550225.
\end{acknowledgments}
