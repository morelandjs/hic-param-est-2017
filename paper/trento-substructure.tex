...
Recent work now shows that hydrodynamic simulations using IP-Glasma initial conditions---with color charge fluctuations calibrated to fit coherent and incoherent $J/\psi$ diffraction measured by the H1 and Zeus experiments at HERA \cite{Aaron:2009aa, Abramowicz:2015mha}---describe collectivity in both small and large collision systems without additional retuning of the nucleon substructure parameters \note.

Model parameters are of course always in some degree of tension; for instance, fitting one observable may require parameter values which degrade the quantitative description of some other observable.
Correspondingly, parameters which provide an optimal description of small-system bulk observables may lead to a sub-optimal description of heavy-ion collision bulk observables or \emph{vice versa}.
It is thus import to look at the experimental data holistically, and to adopt rigorous statistical measures that facilitate systematic model-to-data comparison with meaningful uncertainties.

In this work, we perform a global analysis of soft-sector observables measured in $p$-Pb and Pb-Pb collisions at $\sqrts=5.02$~TeV using established Bayesian methodology to estimate the parameters of a hybrid transport model based on viscous hydrodynamics and Boltzmann transport.
