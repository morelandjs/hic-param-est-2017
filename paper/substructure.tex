...
\title{
  ...
  calibrated to $\boldsymbol p$-Pb and Pb-Pb collisions at \texorpdfstring{$\mathbf{\sqrts=5.02}$}{}~TeV
}
...
The resulting nuclear overlap time $\Delta \tau \lesssim 0.1\ \fmc$ at top RHIC and LHC energies, and thus it is safe to neglect the initial transverse dynamics which occur while the nuclei pass through each other.
...
\end{equation}
and each constituent's position $\mathbf{x_i}$ in Eq.~\eqref{eq:rho} is sampled from a Gaussian radial distribution with standard deviation $r$.
...
Within the eikonal approximation, the initial entropy deposited at midrapidity and proper time $\tau=0^+\ \fmc$ is some function
\begin{equation}
  f: \T_A, \T_B \mapsto \frac{dS}{d^2x_\perp d\eta} \bigg\vert_{\eta=0},
\end{equation}
...
\begin{equation}
  \frac{dS}{d^2x_\perp d\eta}\,\bigg\vert_{\eta=0} \propto \frac{\T_A + \T_B}{2},
\end{equation}
