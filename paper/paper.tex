...
\begin{equation}
  ...
  P(\xv_i; \xv_{A,B}) = \frac{1}{(2\pi r^2)^{3/2}} \exp\left(-\frac{|\xv_i - \xv_{A,B}|^2}{2 r^2}\right),
\end{equation}
...
The protons in this work are described by a \emph{single} density field.
The constituents are simply used as a mechanism to generate deformity and fluctuations.
...
Assuming our protons collide at the sampled impact parameter $\bv$, we assign each proton a participant thickness,
\begin{equation}
...
The initial energy density $e$ deposited at midrapidity $\eta_s=0$ and proper time $\tau_0 \approx 0~\fmc$ is then some function,
\begin{equation}
  e(\xv_\perp, \eta_s=0, \tau_0) = f(\T_A, \T_B),
\end{equation}
of the local participant thicknesses $\T_A$, $\T_B$ in each nucleus.
...
\end{equation}
based on a family of functions known as the generalized mean(s)\footnote{Previous applications of the \trento\ model used the generalized mean ansatz to parametrize the initial entropy density $s(\xv_\perp, \eta_s=0, \tau_0)$, noting its approximate linear relation with the final charged particle density $d\nch/d\eta$ \cite{Moreland:2014oya, Bernhard:2016tnd}. In the present framework, the initial energy density $e(\xv_\perp, \eta_s=0, \tau_0)$ is approximately proportional to $d\nch/d\eta$ as well; hence, the generalized mean works equally well as a parametrization for either quantity. Here we use the energy density to make direct contact with the free-streaming equation Eq.~\eqref{energy-momentum}.}.
...
The physics model thus represents a vector-valued function $f(\xv) = \yv_m$ which maps the parameter values $\xv$ to the simulated observables $\yv_m$.
...
\end{equation}
between each event's initial secondary-parton density ${N_\text{parton} = \int d^2x\,n(\xv)}$ in Eq.~\eqref{energy-momentum} and its final-state event activity, characterized by $\nch$ or $\ntrk$ \cite{Abelev:2013bla, Chatrchyan:2013nka}.
