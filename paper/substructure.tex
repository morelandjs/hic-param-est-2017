...
First, we aimed to establish whether or not our hydrodynamic framework was able to describe $p$-Pb and Pb-Pb bulk observables at $\sqrts=5.02$~TeV simultaneously, i.e.\ we attempted to falsify the framework.
And second, given that the former is true, we wished to obtain estimates for the \emph{true} model parameters given the assumptions of our model and the evidence provided by the experimental data.

Perhaps the most important difference between the present study and previous Bayesian analyses of heavy-ion collision models \note\ is the inclusion of variable nucleon substructure.
We described the QGP initial state using a modified version of the parametric \trento\ model, with new parameters to vary the fluctuating size and shape of each nucleon.
The nucleon was modeled as a cluster of $n_c$ Gaussian constituents (hot spots), and each constituent was described by a Gaussian density profile of width $v$.
We also varied a parameter $w$ which controlled the sampling radius of the constituent positions inside each nucleon and hence the resulting RMS width of the ensemble averaged nucleon density.

The transport dynamics were simulated using an event-by-event multi-stage nuclear collision model described by a pre-equilibrium free-streaming stage coupled to boost-invariant viscous hydrodynamics for the early hot and dense regions of the collision, and a microscopic hadronic afterburner to simulate hadronic interactions .
We parametrized various sources of uncertainty including the duration of the pre-equilibrium free-streaming stage, the temperature dependence of the QGP shear and bulk viscosities, and the switching or particlization temperature where the medium description switches from hydrodynamics to microscopic Boltzmann transport.

%Unfortunately, validating (or invalidating) a hydrodynamic picture of $p$-Pb and Pb-Pb collisions is not as simple as comparing a single model calculation to experimental data.
%Hydrodynamic simulations typically include a number of free parameters which quantify uncertainties in the initial conditions and QGP medium, and these parameters can dramatically affect model predictions.
%It's thus imperative to test all possible combinations of parameters in order to compare the full landscape of physically reasonable model calculations to experimental data.
%If all possible combinations of model parameters fail to describe both collision systems simultaneously, then it's probably safe to say that the underlying assumptions of the chosen framework are either incomplete or invalid.
%Similarly, if a single set of model parameters describes $p$-Pb and Pb-Pb bulk observables simultaneously, then it's compelling evidence that the hydrodynamic framework generalizes to small collision systems.
%
%There is also much more that can be learned about a model by testing all possible parameter combinations as opposed to a single set of values.
%If the model works exceptionally well, but only for a certain range of parameter values, it's evidence that those parameter values are somehow constrained by the data.
%Moreover, rigorous statistical methods may then be used to report parameter estimates with meaningful uncertainties which account for the accuracy of the chosen framework and the resolving power of the experimental data.
