...
\newcommand{\trento}{\textsc{T\raisebox{-0.5ex}{R}ENTo}}
...
We argue that the global agreement of the calibrated model with the experimental data strongly supports the existence of hydrodynamic flow in small collision systems at ultrarelativistic energies, and that the flow produced develops at length scales smaller than a proton.
...
These \emph{ad hoc} gamma random weights are necessary to describe the large fluctuations observed in minimum bias proton-proton collisions, although their physical origin is not yet well understood.
...
A natural first guess for the function $f$ is an arithmetic mean $f \propto (\T_A + \T_B)/2$ of participant nuclear matter $\T_{A,B}$, which is simply a wounded nucleon model up to meaningless factor of two in the overall normalization.
...
We thus sample the distribution and histogram the samples to project the distribution onto one or two dimensions.
Each diagonal panel in Fig.~\ref{fig:posterior} is the distribution of a \emph{single} model parameter (marginalized over all others), and each lower-diagonal panel is the joint distribution of a \emph{pair} of model parameters (marginalized over all others).
The posterior includes a wealth of information.
Here we focus on its most important features, starting with the top-left corner and working our way to the bottom-right.

\subsection{Initial condition parameters}

The \trento\ normalization factor is well constrained ${\text{Norm} = 20.3^{+2.2}_{-2.2}}$, as is the generalized mean parameter $p=0.018^{+0.146}_{-0.159}$ which fixes the scaling of initial QGP entropy deposition.


are well constrained, and we see that the posterior emulator predictions in Fig.~\ref{fig:obs_ppb} and \ref{fig:obs_pbpb} nicely describe the charged particle yields $d\nch/d\eta$ of both collision systems.
This is perhaps one of the strongest tests of the initial condition model.
...
which would not be the case if there was significant tension in fitting the $p$-Pb and Pb-Pb yields, or.
Moreover, our
It the emulator predictions using the posterior value describes both the $p$-Pb and Pb-Pb yields simultaneously (e.g.\ see Figs.~\ref{fig:obs_ppb}).
    We emphasize that this self-consistency is highly non-trivial since the ratio of $p$-Pb to Pb-Pb yields model can only fit both yields simultaneously if the \trento\ entropy deposition parameter $p$ is appropriately tuned.
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    \paddedhline
    Norm              & Normalization factor                 & 20.2           \\
    $p$               & Entropy deposition parameter         & 0.00    \\
    $\sigmaf$         & Nucleon fluctuation std.\ dev.\      &             \\
    $w$               & Nucleon width parameter              & 0.4--1.2 fm     \\
