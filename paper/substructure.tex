...
Within a picture of independent pairwise collisions between the constituents, a Glauber model model may be used to calculate the probability $P_\mathrm{coll}$ that the two protons collide inelastically at impact parameter $b$. In the limit when the number of constituents $n_c \gg 1$, it yields the particularly simple form
\begin{equation}
  \label{eq:pcoll}
...
\end{equation}
and each constituent's position $\mathbf{x_i}$ in Eqn.~\eqref{eq:rho} is sampled from a Gaussian radial distribution of width $r$.
...
Of course the model is \emph{not} a substitute for first principle saturation calculations, and may fail to reproduce nuanced features of \emph{ab initio} saturation theory calculations, such as the existence of short-range gluon field fluctuations \cite{Schenke:2012wb}.
...
\begin{itemize}[leftmargin=2\parindent]
  \item The constituent fluctuations, modeled by the gamma random weights $\gamma_i$ in Eqn.~\eqref{eq:fluctuated_thick}, generate overall nucleon fluctuations which are suppressed by the number of constituents $n_c$ inside the nucleon.
    ...
    We therefore reparametrize the constituent fluctuations using the standard deviation of the effective nucleon fluctuations
    \begin{equation}
  ...
  \begin{equation}
    ...
    w = \sqrt{r^2 + v^2},
  \end{equation}
  visualized in Fig.~\ref{fig:nucleon_schematic}, which approximates the rms radius of the nucleon when the number of sampled constituents is large.
  We thus call $w$ a nucleon ``width'', although for smaller numbers of constituents, the \emph{actual} rms radius of our sampled nucleons can be significantly smaller than our width parameter $w$ due to fluctuations in the nucleon's center of mass, and hence once should account for these difference when discussing the nucleon's rms radius.
  We account for this effect when discussing the nucleon width posterior distribution later in the text.
...
\begin{table*}
  ...
  \begin{ruledtabular}
  \begin{tabular}{lcccc}
    Collision system & Observable & Particle selection & Event bins & Ref. \\
    \paddedhline
    \multirow{2}{*}{Pb-Pb 5.02 TeV} & Yield $d\nch/d\eta$ & chg \& $|\eta| < 0.5$ & centrality: 0--5, 5--10, 10--20, \ldots, 60--70 & \note \\
    \noalign{\smallskip}
    & Flow cumulants $\vnk{n}{2}$ & chg \& $|\eta| < 1$ & centrality: 0--5, 5--10, 10--20, \ldots, 60--70 & \note \\
    \noalign{\smallskip}
    %Mean transverse momentum $\langle p_T \rangle$ & $\pi^\pm$, $K^\pm$, $p\bar p$ &
  \end{tabular}
  \end{ruledtabular}
\end{table*}
...
\subsection{Calibration observables}

We focus on simple nuclear collision observables which are sensitive to the bulk properties of the produced medium and which can be calculated using a relatively small $\mathcal{O}(10^4)$ number of events.
...
The model is computationally intensive to evaluate, ${\sim}10$~min/event per processor core, and thousands of events are required to calculate the simplest observables for a single parameter point.
Moreover, we need to evaluate Eqn.~\eqref{eq:likelihood} \emph{numerous} times in order to sample and visualize the multidimensional posterior distribution.

We therefore follow an established framework for computationally intensive models and train a Gaussian process emulator (GPE) to act as a fast surrogate for direct model evaluation.
The GPE enables essentially instantaneous predictions $\y_m = f(\x)$ which allows us to sample the posterior distribution millions of times.
With the posterior parameter samples $\{\x_\star\}$ in hand, we simply histogram the samples to marginalize over (integrate out) all but one or two model parameters.
We can then plot the flattened distribution to examine the constraints on each individual model parameter.

In order to train the GPE, we first generate a scaffolding of the parameter space using a maximin Latin hypercube design to distribute 500 points in our 15-dimensional parameter space according to the ranges in Tbl.~\ref{tab:design}.
We then run $\mathcal{O}(10^4)$ $p$-Pb and Pb-Pb events at each design point, and calculate the model observables $\y_m$---discussed in the next subsection---from the ensemble of events.
