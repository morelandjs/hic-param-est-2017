...
One approach to investigate the effect of nucleon substructure on hydrodynamic simulations is to replace round protons with composite protons, described by a few salient structure parameters such the number of constituents and each constituent's width, in order to study the effect of each structure parameter on simulated observables \cite{Adler:2013aqf, Mitchell:2016jio, Welsh:2016siu}.
Such sensitivity studies are useful for identifying cause and effect relationships between model inputs and outputs, but lack the ability to constrain nucleon substructure parameters in any kind of global or systematic fashion.

An alternative approach to constrain nucleon substructure parameters, is to leverage information from independent experimental measurements which are particularly sensitive to the average size, shape and fluctuations of the proton.
For instance, measurements by the TOTEM collaboration at $\sqrt{s}=7$~TeV found an unexpected dip in the inelasticity density of $p$-$p$ collisions at nonzero impact parameter \cite{Antchev:2011zz}.
...
Meanwhile, initial condition studies using the IP-Glasma model of color-glass condensate effective field theory demonstrated that these color charge fluctuations leave a lasting imprint on the \mbox{small-x} gluon distribution of the proton and the initial geometry of QGP energy deposition \cite{Schlichting:2014ipa}. Recent work now shows that IP-Glasma initial conditions with color charge fluctuations tuned to fit coherent and incoherent $J/\psi$ diffraction measured by the H1 and Zeus experiments at HERA \cite{Aaron:2009aa, Abramowicz:2015mha} well reproduces the collective flow observables in small and large collision systems without additional fine tuning of the nucleon substructure parameters \note.

More comprehensive efforts to constrain nucleon substructure properties are complicated by the typically large number of free parameters already used in hydrodynamic transport models.
Parameters generally affect multiple observables at once, and intercorrelate in complex ways which make it impossible to tune the parameters by hand.
Evaluating even a single set of parameter values is a computationally intensive process, which generally involves thousands of simulated hydrodynamic events and hundreds of processor hours.

In this work, we take a somewhat different approach to the aforementioned studies and investigate the extent to which nucleon substructure properties can be inferred from a global analysis of p-Pb and Pb-Pb bulk observables at $\sqrts=5.02$~TeV.
Theoretical ingredients of our model which are uncertain, such as the size and shape of the nucleon, the temperature dependence of the QGP transport coefficients, and the duration of pre-equilibrium dynamics, are described by free parameters with probabilistic values that reflect their inherent uncertainty.
...
\end{equation}
equal to the beam-integrated proton density in Eqn.~\eqref{rho}, with each constituent shifted by the appropriate impact parameter offset, and multiplied by a gamma random variable with unit mean and variance $1/k$.
These \emph{ad hoc} gamma random weights are necessary to describe the large fluctuations observed in minimum bias proton-proton collisions, although their exact physical origin is unknown.
...
The minimum distance algorithm, first described in Ref.~\note, uses a simple trick to resample the nucleon positions without modifying the target Woods-Saxon distribution.
...
We therefore choose to model the initial off-equilibrium dynamics of the produced QGP medium using a simple step-function approximation, depicted in Fig.~\ref{fig:freestream}, which free-streams the initial state for proper time $\tau_{fs}$ (zero coupling) before instantaneously switching to viscous hydrodynamics (strong coupling) \note.
...
Under this assumption, it was shown in Refs.~\cite{Broniowski:2008qk, Liu:2015nwa} that the energy-momentum tensor $T^{\mu\nu}$ of partons at midrapidity and time $\tau$ only depends on its spatial distribution at some earlier time $\tau_0$, but not its $p_\perp$-distribution (which could depend on $\mathbf{x}$).
...
We model the shear viscosity $\eta$ and bulk viscosity $\zeta$ as free parameters in our study and fix the remaining transport coefficients $\{\tau_\Pi, \delta_{\Pi\Pi}, \lambda_{\Pi\pi}, \tau_\pi, \delta_{\pi\pi}, \phi_7, \tau_{\pi\pi}, \lambda_{\pi\Pi}\}$ using analytic results derived in the limit of small but finite masses \note.

The hydrodynamic equations of motion are closed using an equation of state (EoS) to relate the energy density $e$ and pressure $P$ of the produced medium.
We use a parametrization for $P(e)$ that matches continuum-extrapolated lattice calculations for (2+1)-flavor QCD by the HotQCD collaboration \note\ to a hadron resonance gas EoS in the temperature interval $165 \le T \le 200$~MeV.

Latest results for the HotQCD trace anomaly calculated using a HISQ/tree fermion action are in good agreement with previous calculations by the Wuppertal-Budapest group using a stout fermion action \note.

Subsequent lattice calculations for (2+1+1)-flavors to investigate the effect of thermalized charm quarks, although studies indicate that the effect on hydrodynamic observables should be relatively small.
%We implement the hydrodynamic equations using the well-tested \texttt{VISH2+1} viscous hydrodynamics code \note, which solves equations .
...
We parametrize the temperature dependence of the specific shear viscosity $\eta/s$ using a modified linear ansatz
\begin{equation}
...
For the specific bulk viscosity $\zeta/s$, we use an unnormalized Cauchy distribution
\begin{equation}
