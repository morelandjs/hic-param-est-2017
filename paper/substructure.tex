...
\subsection{Simulated and measured observables}
...
\begin{itemize}[leftmargin=2\parindent, itemsep=0pt]
  ...
  \item Two-particle flow cumulants $\vnk{n}{2}$ for $n=2,3,4$\\ (charged particles, $0.2 < p_T < 5.0$~GeV \cite{Adam:2016izf}, and $0.3 < p_T < 3.0$~GeV \cite{Chatrchyan:2013nka}).
\item Four-particle flow cumulant $\vnk{2}{4}$\\ (charged particles, $0.2 < p_T < 5.0$) \cite{Adam:2016izf}.
...
The Pb-Pb four-particle cumulants $\vnk{n}{4}$ at $\sqrts=5.02$~TeV which are noticeably absent \emph{are} currently available, but we exclude from the present calibration for statistical reasons.

Most of the calibration observables listed in Tbl.~\ref{tab:observables} are calculated as a function of collision centrality, where centrality is defined as some measure of the underlying event activity, e.g.\ the charged particle yield in a given rapidity window.
...
However, for some observables such as $p$-Pb mean $p_T$ and flow cumulants, the experiments use a special high-multiplicity trigger to select rare, ultra-central events according to the number of charged particles produced or detector tracks offline \note.
...
\begin{equation}
  \ntrk \sim \nch \propto \int d^2x\, n(\x)
\end{equation}
between each event's initial transverse parton density $n(\x)$ and its final-state event activity, characterized by $\nch$ or $\ntrk$.

and the event's initial parton density $n(\x)$ in Eqn.~\eqref{energy-momentum}, and run a high-multiplicity trigger on the relative event activity
\begin{equation}
  \mathrm{bin~low} \le \nch / \langle \nch \rangle \le \mathrm{bin~high}.
\end{equation}
