...
def validate_grid_extrap(system=default_system):
    ...
    observables = [
        ('dNch_deta', None, r'$dN_\mathrm{ch}/d\eta$'),
        ('mean_pT', None, r'mean $p_T$'),
        ('vnk', (2, 2), r'$v_2\{2\}$'),
        ('vnk', (3, 2), r'$v_3\{2\}$')
    ]
    ...
    for ax, (obs, subobs, label) in zip(axes, observables):
        ...
        # plot line y=x
        min_xy = min(*ax.get_xlim(), *ax.get_ylim())
        max_xy = max(*ax.get_xlim(), *ax.get_ylim())
        ax.plot([min_xy, max_xy], [min_xy, max_xy], color=offblack, zorder=0)
...
def pc_scatter(system=default_system):
    ...
    for n, (z, dz, zval) in enumerate(zip(Z, dZ, Zval), start=1):
        ...
        # validation
        ax.scatter(*Xval.T, zval, color=green)

        # emulator
        ax.scatter(*X.T, z, color=blue)
        ax.scatter(*X.T, z - 2*dz, color=orange)
        ax.scatter(*X.T, z + 2*dz, color=orange)

        # labels
        ax.set_xlabel('grid scale')
        ax.set_ylabel('parton structure')
