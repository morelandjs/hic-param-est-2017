...
Moreover, their posterior values nicely describe the calibration observables of both collision systems as evidenced by Figs.~\ref{fig:obs_pbpb} and \ref{fig:obs_ppb}.
...
For example, as the entropy deposition parameter $p \rightarrow \pm\infty$, particle production scales like the local maximum (minimum) of the nuclear overlap density.
This strongly affects the ratio of the predicted $p$-Pb and Pb-Pb yields and consequently---absent any additional information---excludes \emph{most} values of the parameter $p$.
It just so happens that the small range of $p$-values needed to describe the aforementioned yield ratio are the same values needed to describe all of the calibration observables in the present study, \emph{and} numerous Pb-Pb observables at $\sqrts=2.76$~TeV \note.
This work thus reaffirms an empirical scaling law reported in several previous studies \note, that entropy deposition at midrapidity in ultrarelativistic nuclear collisions scales approximately as
\begin{equation}
...
For instance, a generalized mean described by $p=0.05$ is well within our 90\% credible region, but it is not equal to the geometric mean in Eqn.~\ref{eq:geometric_mean}.
This approximate form, nevertheless, has been shown to mimic the scaling behavior of several saturation-based theory calculations in high-energy QCD \cite{Bernhard:2016tnd}, and thus it appears to be consistent with first-principle theoretical expectations.
...
For example, the single highest posterior probability region of parameter space prefers a nucleon width $w=0.92$~fm, constituent width $v=0.43$~fm and constituent number $n_c=6$.
...
The studies find that the measured coherent and incoherent $J/\Psi$ spectra at HERA prefer a compact gluon distribution inside each nucleon with an RMS radius $R_g = 0.4$~fm, which is approximately \emph{half} the analogous nucleon length scale in our model and clearly a significant discrepancy.

The posterior constituent number $n_c = 6.2_{-3.1}^{+2.8}$ clearly disfavors $n_c=1$ constituent, although there is no strong peak for $n_c > 1$ constituents.
This suggests that initial condition models which lack nucleon substructure will perform provide a worse description $p$-Pb and Pb-Pb bulk observables simultaneously which is perhaps at odds with 


\begin{table}[b]
  \caption{Maximum a posteriori (MAP) parameters.}
  \begin{ruledtabular}
  \begin{tabular}{lll}
    Parameter         & Description                        & Value\\
    \paddedhline
    Norm              & Normalization factor                 & 20.            \\
    $p$               & Entropy deposition parameter         & 0.0            \\
    $k$               & Constituent gamma fluct.\ shape      & 0.19           \\
    $w$               & Nucleon width parameter              & 0.92 fm        \\
    $n_c$             & Number of nucleon constituents       & 6              \\
    $v$               & Constituent width                    & 0.43 fm        \\
    $\dmin$           & Minimum inter-nucleon distance       & 0.81 fm        \\
    $\taufs$          & Free-streaming time                  & 0.37 fm/$c$    \\
    $\eta/s$ min      & Minimum value of $\eta/s$ (at $T_c$) & 0.11           \\
    $\eta/s$ slope    & Slope of $\eta/s$ above $T_c$        & 1.6 GeV$^{-1}$ \\
    $\eta/s$ crv      & Curvature of $\eta/s$ above $T_c$    & -0.29          \\
    $\zeta/s$ max     & Maximum value of $\zeta/s$           & 0.032          \\
    $\zeta/s$ width   & Width of $\zeta/s$ peak              & 0.024 GeV      \\
    $\zeta/s$ $T_0$   & Temperature of $\zeta/s$ maximum     & 175 MeV        \\
    $T_\text{switch}$ & Switching/particlization temp.       & 151 MeV        \\
  \end{tabular}
  \end{ruledtabular}
  \label{tab:design}
\end{table}
...
The posterior in Fig.~\ref{fig:posterior} is the distribution of true parameter values, given the assumptions of our model and the evidence provided by the experimental data.
Correspondingly, the probability of nature ``choosing'' any specific set of parameter values is \emph{zero}; we can only assign a probability to a finite region of parameter space.
Nevertheless, we can define the \emph{maximum a posteriori probability} (MAP) estimate as the single most probable neighborhood (i.e.\ point) in parameter space, equal to the mode of the distribution.

We estimate the MAP parameters for the posterior in Fig.~\ref{fig:posterior} and run \order{6} events for $p$-Pb and Pb-Pb (minimum bias and multiplicity triggered) 

\begin{fullpage}
{\centering\includegraphics{observables_map}\par}
\captionof{figure}{\label{fig:obs_map} Model calculations using the \emph{maximum a posterior} (MAP) parameters compared to experiment. Colored lines are model calculations for $p$-Pb collisions (left) and Pb-Pb collisions (right) at $\sqrts=5.02$~TeV. Points with error bars are the experimental data with statistical uncertainties, and gray bands their corresponding systematic uncertainties, from CMS \cite{Chatrchyan:2013nka} and ALICE \cite{Adam:2015ptt, Adam:2016izf, Adam:2014qja, Abelev:2013bla}. The ratio axes show the ratio of model over data where where available with gray bands indicating $\pm 10\%$.}
\end{fullpage}
...
\begin{figure*}
  ...
  \caption{}
  \label{fig:validation_PbPb5020}
\end{figure*}

\begin{figure*}
  \includegraphics{validation_pPb5020}
  \caption{}
  \label{fig:validation_pPb5020}
\end{figure*}
