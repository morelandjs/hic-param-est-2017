...
\begin{frame}{\scshape Posterior nucleon shape}
  \begin{columns}
    ...
    \begin{column}{.55\textwidth}
      \begin{itemize}
        \item Parton sampling radius (approx nucleon width) is not well constrained\\[2ex]
        \item Analysis prefers small partons,\\
          $w_\text{parton} = 0.38_{-0.18}^{+0.21}$~fm\\[2ex]
        \item[\small\faicon{exclamation-triangle}] Results are likely sensitive to assumed
          details of the model, e.g.\ equation of state, pre-equilibirum dynamics and viscous
          corrections to freezeout
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
