...
It just so happens that the small range of $p$-values needed to describe this yield ratio are the same values needed to describe all of the calibration observables in the present study, \emph{and} numerous Pb-Pb observables at $\sqrts=2.76$~TeV \note.
...
Meanwhile, the posterior on the constituent number $n_c$ is not sharply peaked.
...
The \trento\ model mimics saturation-based initial condition models, and saturation models tend to produce ``proton-sized'' fireballs in $p$-Pb collisions \note.
...
Saturation-based models are therefore unable to describe the significant flow measured in high-multiplicity $p$-Pb collisions without nucleon substructure, or alternatively, some other source of additional correlations \note.
...
The full posterior in Fig.~\ref{fig:posterior} is the distribution of true parameter values, given the assumptions of our model and the evidence provided by the experimental data.
...
We list the posterior MAP parameters in Tbl.~\ref{tab:map_param}.
The values are estimated using a m

and run \order{6} events minimum-bias and multiplicity triggered $p$-Pb and Pb-Pb events at $\sqrts=5.02$~TeV.
